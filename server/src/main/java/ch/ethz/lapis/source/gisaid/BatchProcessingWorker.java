...
public class BatchProcessingWorker {
                    ...
                    if (nre != null) {
                        ...
                        if (entry.getSeqAligned() != null) {
                            ...
                            String nucSubstitutions = nucMutations.stream()
                                    ...
                                    .map(m -> String.valueOf(refGenome.getNucleotideBase(m.getPosition())) + m.getPosition() + m.getMutation())
                ...
                try (ResultSet rs = statement.executeQuery()) {
                    while (rs.next()) {
                        ...
                        if (Objects.equals(entry.getStrain(), rs.getString("strain"))
                                && Objects.equals(entry.getDate(), rs.getDate("date") != null ? rs.getDate("date").toLocalDate() : null)
                                && Objects.equals(entry.getDateOriginal(), rs.getString("date_original"))
                                && Objects.equals(entry.getDateSubmitted(), rs.getDate("date_submitted") != null ? rs.getDate("date_submitted").toLocalDate() : null)
                                ...
                                && (entry.getSubmitterInformation() == null || (
                                        Objects.equals(entry.getSubmitterInformation().getOriginatingLab(), rs.getString("originating_lab"))
                                                && Objects.equals(entry.getSubmitterInformation().getSubmittingLab(), rs.getString("submitting_lab"))
    ...
    private Map<String, NextalignResultEntry> runNextalign(
        ...
        for (String gene : genes) {
            FastaFileReader geneFastaReader = new FastaFileReader(outputPath.resolve("nextalign.gene." + gene + ".fasta"),
                    false);
