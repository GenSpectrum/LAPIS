...
class SequencesStreamer(
    ...
    private fun streamFasta(
        ...
        response.outputStream.writer().use { stream ->
            val nonNullData = sequenceData.filter { (_, sequences) -> sequences.values.any { it != null } }
            if (singleSequenceEntry) {
                ...
                TODO()
                nonNullData.forEach { (sequenceKey, sequences) ->
                    sequences.forEach { (segmentName, sequence) ->
                        if (sequence != null) {
