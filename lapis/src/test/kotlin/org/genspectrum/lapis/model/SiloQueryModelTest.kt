...
class SiloQueryModelTest {
    ...
    private val singleSegmentedReferenceGenomeSchema = ReferenceGenomeSchema(
        nucleotideSequences = listOf(ReferenceSequenceSchema("someSequenceName")),
        genes = emptyList(),
    )
    private val multiSegmentedReferenceGenomeSchema = ReferenceGenomeSchema(
    ...
    @Test
    fun `lineage definition uses the public view field`() {
        every { siloClientMock.getLineageDefinition("publicLineage") } returns emptyMap()

        underTest.getLineageDefinition("publicLineage")

        verify(exactly = 1) { siloClientMock.getLineageDefinition("publicLineage") }
    }
    ...
    fun `GIVEN multiSegmented and fields = mutation WHEN get nuc mutations THEN sequence name is null`() {
        ...
        every { siloFilterExpressionMapperMock.map(any<CommonSequenceFilters>()) } returns True
        underTest = createSiloQueryModel(multiSegmentedReferenceGenomeSchema)
