...
use crate::{DatabaseConfig, ExecutorService, SeqCompressor};
use chrono::Local;
use md5::{Digest, Md5};
...
pub fn hash_uppercase(db_config: &DatabaseConfig, seq_compressor: &mut SeqCompressor) {
    // Load from file
    ...
    let mut outdated = Arc::new(Mutex::new(Vec::new()));
    let mut missing = Arc::new(Mutex::new(Vec::new()));
    let mut iteration = 0;
