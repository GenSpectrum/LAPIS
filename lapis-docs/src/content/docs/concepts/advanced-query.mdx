...
:::note When passing single quotes in curl they must be escaped correctly, for example the statement:
...
To search for empty fields (fields that are `null` or empty) use the `ISNULL` operator:
