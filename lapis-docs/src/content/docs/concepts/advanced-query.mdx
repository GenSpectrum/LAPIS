---
title: Advanced query
description: Generalized advanced queries
---

import { OnlyIf } from '../../../components/OnlyIf.tsx';
import { hasFeature } from '../../../config.ts';
import { Aside } from '@astrojs/starlight/components';

{/* prettier-ignore */}
<OnlyIf condition={!hasFeature('generalizedAdvancedQuery')}>
:::note
This feature is not enabled for this LAPIS instance.
It must be enabled in the `features` section of the database configuration by the maintainer of the instance.
:::
</OnlyIf>

<Aside type='note' title='TLDR'>
    Advanced queries can be used to __filter sequences and metadata__ and can be passed to the server through the
    __query parameter `advancedQuery`__. Compared to standard filters, advanced queries allow for __more complex
    combinations of filters__.
</Aside>

Advanced queries should allow an arbitrary combination of filters. Standard LAPIS filters take the form
`filter1 AND filter2 AND ... filterN` but they cannot query for more custom cases such as `filter1 OR (filter2 AND NOT filter3)`,
the advanced queries feature allows such combinations and the creation of more custom queries.

The formal specification of the query language is available
[here](https://github.com/GenSpectrum/LAPIS/blob/main/lapis/src/main/antlr/org/genspectrum/lapis/model/advancedqueryparser/AdvancedQuery.g4)
as an ANTLR v4 grammar.
In following, we provide an informal description and examples.
The respective
[unit test](https://github.com/GenSpectrum/LAPIS/blob/main/lapis/src/test/kotlin/org/genspectrum/lapis/model/AdvancedQueryFacadeTest.kt)
provides a full list of possible atomic queries.

:::tip
For those readers familiar with the [CoV-Spectrum variant query](../concepts/variant-query) feature, advanced queries can be
seen a superset of variant queries, in which not only variant information can be queried but also metadata information.
:::

## Features

Advanced queries are case-insensitive. All operators and field names can be written in upper or lower case.
Only the values of metadata fields are case-sensitive.

### Variant Queries

We support mutation and insertion queries for both nucleotide and amino acid sequences, see the [mutation filter](../concepts/mutation-filters)
page for more details. Note the addition of the `MAYBE` operator to query [ambiguous nucleotide symbols](../concepts/ambiguous-symbols).

### Metadata Queries

Standard metadata queries take the form `metadataField=query`, for example

```
country=Ghana
```

Note that if the metadata field does not only contain letters and numbers it must be enclosed in single quotes, for example

```
country='United States of America'
```

To include a literal single quote inside a quoted value, escape it with a backslash. To include a literal backslash, escape it as `\\`:

```
country='Côte d\'Ivoire'
```

When using regex-based operators (for example `field.regex='…'`),
remember that backslash is also the escape character inside quoted strings.
Regex patterns that require a literal backslash, including standard regex escape sequences such as `\d`, `\w`,
or escaped metacharacters like `\{`, must therefore use doubled backslashes in the query.
For example, write `field.regex='Basel\\{1,2\\}'` in the query language to obtain the regex pattern `Basel\{1,2\}`,
and `field.regex='\\d+'` to obtain the pattern `\d+`.

To search for empty fields (fields that are `null`) use the `IsNull` operator:

```
IsNull(host)
```

For dates and numbers (int or float) we allow queries for ranges, using the `>=` and `<=` operators, for example:

```
date>=2021-01-01
```

```
date<=2021-12-31
```

For string fields we also allow regex search. To use the regex substring search on a metadata string field you must append `.regex` to the end of the metadata field name
and enclose the query in single quotes:

```
host.regex='.*bos.*'
```

For regex searches the advanced queries use the [google/re2](https://github.com/google/re2/wiki/Syntax) regex syntax.

Boolean fields can be queried using `true` and `false` (again case-insensitive) values, for example:

```
isLabHost=true
```

```
isLabHost=false
```

### Boolean operators

The query language understands Boolean logic. Expressions can be connected with `&` (and), `|` (or) and `!` (not).
Both `&` and `AND` are recognized as `and`, `|` and `OR` are recognized as `or`, and `!` and `NOT` are recognized as `not`.
Parentheses `(` and `)` can be used to define the order of the operations.

We also add a custom syntax `N-of` and `exactly-N-of` to match sequences for which at least or exactly `N` out of a list of expressions are fulfilled.
The syntax is as follows, where `expr1`, ..., `expr5` are any valid expressions:

```
[3-of: expr1, expr2, expr3, expr4, expr5]
```

```
[exactly-2-of: expr1, expr2, expr3, expr4]
```

## Examples

Advanced queries can be sent in GET and POST requests just like standard filters.

- GET: https://lapis.cov-spectrum.org/open/v2/sample/aggregated?advancedQuery=501T%20and%20country=Switzerland

- POST:
    ```bash
    curl -X POST "https://lapis.cov-spectrum.org/open/v2/sample/aggregated" \
          -H "Content-Type: application/json" \
          -d '{"advancedQuery": "501T and country=Switzerland"}'
    ```

Some example queries that can be plugged into the `advancedQuery` parameter:

- Get the sequences with the nucleotide mutation 300G, without a deletion at position 400 and either the AA change S:123T
  or the AA change S:234A:

    ```
    300G & !400- & (S:123T | S:234A)
    ```

    This can also be written as

    ```
    300G AND NOT 400- AND (S:123T OR S:234A)
    ```

- Get all sequences from the USA that do not have cows as a host and that also have the mutation 300G:

    ```
    NOT host='bos taurus' AND 300G AND country=USA
    ```

- Get the sequences with at least 3 out of five mutations/deletions:

    ```
    [3-of: 123A, 234T, S:345-, ORF1a:456K, ORF7A:100-]
    ```

- Get the sequences that fulfill exactly 2 out of 4 conditions:

    ```
    [exactly-2-of: 123A & 234T, !234T, S:345- | S:346-, [2-of: 222T, 333G, 444A, 555C]]
    ```

:::note
Don't forget to encode/escape the query correctly. In JavaScript, this can be done with the
[encodeURIComponent()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent)
function.
:::

## Validating Queries

LAPIS provides a `/query/parse` endpoint that allows you to validate and parse advanced queries without executing them. This is useful for:

- Validating query syntax before sending data requests
- Understanding how LAPIS interprets your query
- Debugging complex queries

### Endpoint

`POST /query/parse`

### Request Format

```json
{
  "queries": ["query1", "query2", ...],
  "doFullValidation": false
}
```

**Parameters:**

- `queries` (required): An array of query strings to validate
- `doFullValidation` (optional, default: `false`): Controls the depth of validation
    - When `false`: Only validates query syntax and metadata field names (fast)
    - When `true`:
      Performs full validation against RhyDB, including mutation positions and gene names (more thorough but slower)

**Use `doFullValidation: true` when:**

- Validating user-submitted queries to be sure that they will execute successfully
- One-off validation of queries before storing them for later use

**Use `doFullValidation: false` when:**

- Quickly checking syntax during query construction (e.g. for immediate feedback when a user types a query in a UI)
- Only need to verify basic structure

### Response Format

The endpoint returns a discriminated union for each query. Each result has a `type` field that indicates whether parsing succeeded or failed.

**Success case:**

```json
{
  "data": [
    {
      "type": "success",
      "filter": {
        "type": "StringEquals",
        "column": "country",
        "value": "USA"
      }
    }
  ],
  "info": { ... }
}
```

**Failure case:**

```json
{
  "data": [
    {
      "type": "failure",
      "error": "Parse error at position 8: unexpected token"
    }
  ],
  "info": { ... }
}
```

### Example

Validate multiple queries in a single request:

```bash
curl -X POST "https://lapis.cov-spectrum.org/open/v2/query/parse" \
  -H "Content-Type: application/json" \
  -d '{"queries": ["country=USA & 501T", "invalid!!!"]}'
```

Response:

```json
{
  "data": [
    {
      "type": "success",
      "filter": {
        "type": "And",
        "children": [
          {
            "type": "StringEquals",
            "column": "country",
            "value": "USA"
          },
          {
            "type": "NucleotideSymbolEquals",
            "sequenceName": "main",
            "position": 501,
            "symbol": "T"
          }
        ]
      }
    },
    {
      "type": "failure",
      "error": "Parse error: unexpected input"
    }
  ],
  "info": { ... }
}
```

The endpoint processes multiple queries in a single request, returning results for each query independently. Valid queries show the parsed filter structure, while invalid queries return descriptive error messages.
