...
import NucleotideMutationsExample from '../../../components/MutationFilters/NucleotideMutationsExample.astro';
...
A nucleotide mutation has the format <NucleotideMutations /> <SegmentNames />
A `<base>` can be one of the four nucleotides `A`, `T`, `C`, and `G`. It can also be `-` for deletion and `N` for unknown.
**Example:** <NucleotideMutationsExample />
