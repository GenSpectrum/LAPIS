...
- `--lapis.viewsConfig.path`:
  The path to the views config file that defines LAPIS URL prefixes, base SILO queries, database configs, and reference genomes.
  Database config and reference genome paths are resolved relative to the views config file.
...
- `--lapis.docs.url`:
  If set, each view's landing page will show a "Documentation" link to the provided value.
  It is meant to point to your self-hosted version of this LAPIS documentation.
  If not set, the "Documentation" link will not be shown.
...
- the `lapis.viewsConfig.path` parameter: `/workspace/views.yaml`.

:::note
You simply need to mount the views config and the files it references into the container
