...
The `sarsCoV2VariantQuery` is a specialized query language for SARS-CoV-2 instances, while the `generalizedAdvancedQuery` feature can be used for all instances.
...
<OnlyIf condition={hasFeature('generalizedAdvancedQuery')}>
    See [advanced queries](../../concepts/advanced-query).
</OnlyIf>
