...
description: Reference for how to configure LAPIS and RhyDB
---
...
LAPIS and RhyDB need a `database_config.yaml`.
It's main purpose is to define the database schema for the sequence metadata.
See the [tutorial](../tutorials/start-lapis-and-rhydb#writing-configuration) for an example,
...
| primaryKey    | string | true     | The field that serves as the primary key in RhyDB for the data.                                       |
| dateToSortBy  | string | false    | The field used to sort the data by date. Queries on this column will be faster.                       |
| partitionBy   | string | false    | The field used to partition the data. Used by RhyDB for overall query optimization.                   |
...
RhyDB will partition the data according to the lineage, which will speed up queries,
...
RhyDB internally stores precomputed bitmaps for those columns so that a query on that column becomes a trivial lookup.

:::tip
Generating an index makes most sense for columns with many equal values,
since it increases the compression ratio and thus decreases memory consumption of RhyDB.
:::
...
| Key                   | Type | Required | Description                                                       |
| --------------------- | ---- | -------- | ----------------------------------------------------------------- |
| siloClientThreadCount | int  | false    | How many threads (connections) the RhyDB client uses. Default: 64 |
