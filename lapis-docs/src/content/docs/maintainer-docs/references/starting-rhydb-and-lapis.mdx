---
title: Starting RhyDB and LAPIS
description: Reference on how to start the RhyDB and LAPIS applications
---

:::caution
Some parts of this page might be outdated.
:::

:::note
The [corresponding tutorial](../tutorials/start-lapis-and-rhydb)
...
RhyDB can be started in two modes:

- `preprocessing`: Starts the RhyDB preprocessing that prepares the data for the RhyDB API server.
- `api`: Starts the RhyDB API server that responds to RhyDB queries.
...
To start the RhyDB preprocessing you need to provide the argument `--preprocessing`.
...
- `--preprocessingConfig`:
  The path to the [preprocessing config](../references/preprocessing#preprocessing-config) file that RhyDB should use,
  e.g. `~/LAPIS/preprocessing_config.yaml`
- `--databaseConfig`:
  The path to the [database config](../references/database-configuration) file that RhyDB should use,
  e.g. `~/LAPIS/database_config.yaml`.

If those parameters are not provided, RhyDB will try to read the files from its current working directory.
...
To start the RhyDB API server you need to provide the argument `--api`.
...
- YAML file `./runtime_config.yaml` (in RhyDB's working directory) with the key `dataDirectory`,
- the default value `./output/` (in RhyDB's working directory).
...
- `--silo.url`: The URL where RhyDB is running, e.g. `http://localhost:8081`.
...
and start the container with `--silo.url=<your RhyDB URL>`.
:::
