---
title: Start LAPIS and RhyDB
description: Tutorial to start LAPIS and RhyDB with Docker
---
...
Every LAPIS instance needs to be backed by a RhyDB instance, that acts as data source.
RhyDB could be operated stand-alone.
LAPIS is meant as a layer of convenience and abstraction around RhyDB.

We provide Docker images of RhyDB and LAPIS that are ready to use.
...
Both LAPIS and RhyDB need to know which metadata columns are available in the dataset.
Furthermore, you need to define which column acts as primary key
and which column should be used to generate partitions in RhyDB.
...
RhyDB expects fasta files (possibly compressed via zstandard or xz)
...
Now RhyDB needs to know where it can find those files.
...
Since you don't provide the input directory explicitly, RhyDB will fall back to the default `/data`.
...
- start RhyDB in the `preprocessing` mode
...
That result has to be provided to RhyDB in the next step.

### Starting RhyDB

To start the RhyDB api, you have to:

- start RhyDB in the `api` mode,
- expose port 8081,
...
Now RhyDB should be available at http://localhost:8081
and http://localhost:8081/info should show that RhyDB contains sequences.
...
- provide LAPIS with the RhyDB URL.
